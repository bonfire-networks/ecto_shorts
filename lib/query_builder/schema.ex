defmodule EctoShorts.QueryBuilder.Schema do
  ...
  def create_schema_filter({filter_field, val}, schema, query) do
    cond do
      ...
      true ->
        Logger.warning("[EctoShorts] `#{inspect filter_field}` is not a recognised filter or field for `#{schema.__schema__(:source)}` where you attempted to filter by: #{inspect val}")
